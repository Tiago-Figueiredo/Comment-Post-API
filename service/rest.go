...
func (svc *RestApiService) handleGetPostByPostId(w http.ResponseWriter, r *http.Request) {
	...
	vars := mux.Vars(r)
	postIdToFetch, err := strconv.ParseUint(vars["id"], 10, 64)
	if err != nil {
		prepareJsonResponse(
			w,
			&AckJsonResponse{Message: fmt.Sprintf("wrong id path variable: %s", vars["id"]), Status: http.StatusBadRequest},
			http.StatusBadRequest)
		return
	}

	post, err := svc.postRepository.GetById(postIdToFetch)

	if err != nil {
		prepareJsonResponse(
			w,
			&AckJsonResponse{Message: fmt.Sprintf("Post with id: %d does not exist", postIdToFetch), Status: http.StatusNotFound},
			http.StatusNotFound)
		return
	}

	prepareJsonResponse(w, post, http.StatusOK)
}

func prepareJsonResponse(w http.ResponseWriter, message interface{}, status int){
	w.Header().Set("Content-Type", "application/json")
...
func (svc *RestApiService) handleGetCommentsByPostId(w http.ResponseWriter, r *http.Request) {
	...
	// Given that this project uses gorilla/mux as a router you can access the path params with following code:
	vars := mux.Vars(r)
	commentIdToFetch, err := strconv.ParseUint(vars["id"], 10, 64)
	if err != nil {
		prepareJsonResponse(
			w,
			&AckJsonResponse{Message: fmt.Sprintf("wrong id path variable: %s", vars["id"]), Status: http.StatusBadRequest},
	...
	comments:= svc.commentRepository.GetAllByPostId(commentIdToFetch)

	prepareJsonResponse(w, comments, http.StatusOK)
}
...
func (svc *RestApiService) handleAddComment(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&comment); err != nil {
		prepareJsonResponse(
			w,
			&AckJsonResponse{Message: "could not deserialize comment json payload", Status: http.StatusBadRequest},
			http.StatusBadRequest)
		return
	}
	...
	commentStored, err := svc.commentRepository.GetById(comment.Id)
	if commentStored != nil && err == nil {
		prepareJsonResponse(
			w,
			&AckJsonResponse{Message: fmt.Sprintf("Comment with id: %d already exists in the database", comment.Id), Status: http.StatusBadRequest},
			http.StatusBadRequest)
		return
	}

	err = svc.commentRepository.Insert(comment)
	if err != nil {

	}

	prepareJsonResponse(w,
		&AckJsonResponse{Message: fmt.Sprintf("comment id: %d successfully added", comment.Id), Status: http.StatusOK},
		http.StatusOK)
}
